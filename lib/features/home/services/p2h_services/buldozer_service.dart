...
class BuldozerService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<void> submitP2hBl(Map<String, dynamic> requestData, BuildContext context) async {
    try {
      // 1. Add documents to respective collections
      final aroundUnitRef = await _firestore.collection('aroundunits').add({
        'ku': requestData['ku'],
        'kai': requestData['kai'],
        'lotk': requestData['lotk'],
        'lodk': requestData['lodk'],
        'lopk': requestData['lopk'],
        'lohk': requestData['lohk'],
        'fd': requestData['fd'],
